...
class CourseRearrangeConfig(Document):
    """Stores Config data auto-created from CourseRearrange questions"""
...
class CourseRearrange(Document):
    ...
    def save(self, *args, **kwargs):
        """Override save to auto-update/create CourseRearrangeConfig"""
        self.clean()  # run validation

        # Save CourseRearrange first
        result = super(CourseRearrange, self).save(*args, **kwargs)

        # Update CourseRearrangeConfig
        config = CourseRearrangeConfig.objects.first()
        if not config:
            config = CourseRearrangeConfig()
    ...
    def to_json(self):
        """Convert CourseRearrange document to dict/JSON"""
        return {
            "id": str(self.id),
            "title": self.title,
            "prompt": self.prompt,
